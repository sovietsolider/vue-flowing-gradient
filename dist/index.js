...
// src/composables/useWebGLRenderer.ts
var vertexShader = `
...
`;
function useWebGLRenderer(config) {
  const configValue = vue.computed(() => {
  ...
  const rendererRef = vue.ref(null);
  const programRef = vue.ref(null);
  const meshRef = vue.ref(null);
  const rafRef = vue.ref(0);
  const normalizedColors = vue.computed(() => {
    const config2 = configValue.value;
    if (!config2 || !config2.color1 || !config2.color2 || !config2.color3) {
    ...
    return {
      color1: normalizeRgb(config2.color1),
      color2: normalizeRgb(config2.color2),
      color3: normalizeRgb(config2.color3)
    };
  });
  const handleResize = () => {
    var _a, _b;
  ...
  vue.onMounted(() => {
    ...
    const plane = new ogl.Plane(gl, { width: 2, height: 2 });
    ...
    const program = new ogl.Program(gl, {
      ...
      uniforms: {
        ...
        u_type: {
          value: GRADIENT_TYPE_NUMBER[(_a = configValue.value.type) != null ? _a : "animated"]
        },
    ...
    const mesh = new ogl.Mesh(gl, { geometry: plane, program });
    meshRef.value = mesh;
  ...
  vue.onUnmounted(() => {
    ...
    const gl = (_a = rendererRef.value) == null ? void 0 : _a.gl;
    if (((_b = programRef.value) == null ? void 0 : _b.program) && gl) {
  ...
  vue.watch(
    ...
    () => {
      ...
      program.uniforms.u_type.value = GRADIENT_TYPE_NUMBER[(_a = configValue.value.type) != null ? _a : "animated"];
...
var _sfc_main = /* @__PURE__ */ vue.defineComponent({
  ...
  setup(__props, { expose: __expose }) {
    ...
    const config = vue.computed(() => {
      const normalized = __spreadValues({}, DEFAULT_CONFIG);
      if (props.config) {
        ...
        if (props.config.speed !== void 0) normalized.speed = props.config.speed;
        if (props.config.scale !== void 0) normalized.scale = props.config.scale;
        if (props.config.type) normalized.type = props.config.type;
        if (props.config.noise !== void 0) normalized.noise = props.config.noise;
      }
      return normalized;
    });
...
function _sfc_render(_ctx, _cache, $props, $setup, $data, $options) {
  return vue.openBlock(), vue.createElementBlock(
    ...
    {
      ref: "canvasRef",
      class: vue.normalizeClass($setup.cn("w-full h-full block select-none touch-none", $props.className)),
      style: {
        width: "100%",
        height: "100%",
        display: "block"
...
var GradFlow_default = /* @__PURE__ */ export_helper_default(_sfc_main, [["render", _sfc_render], ["__file", "/home/anatoliy/WORK/my-packages/gradflow/src/components/GradFlow.vue"]]);
...
exports.GRADIENT_TYPE_NUMBER = GRADIENT_TYPE_NUMBER;
exports.GradFlow = GradFlow_default;
